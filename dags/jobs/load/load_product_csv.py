...
def load_product_csv(file_path: str) -> DataFrame:
...
def check_data(df: DataFrame) -> bool:
    ...
    with open(control_file, "r") as f:
        ...
        last_updated_str = control_data["last_updated"]["products"]
...
def load_to_delta(df: DataFrame) -> None:
    ...
    df.write.format("delta").mode("overwrite").save("hdfs://10.128.0.2:9000/delta/product")  # Thay đổi IP cho phù hợp

if __name__ == "__main__":
    # Example usage
    file_path = "/opt/airflow/files/transactions/products.csv"
    df = load_product_csv(file_path)
    if check_data(df):
        load_to_delta(df)
