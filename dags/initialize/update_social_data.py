...
with DAG(
    ...
    catchup=False,
    description='Full ETL pipeline from raw CSV to bronze, silver, and gold layers for social media data',
    tags=['social', 'bronze', 'silver', 'gold']
) as dag:
    ...
    load_to_bronze = BashOperator(
        ...
        /opt/airflow/jobs/social_media/bronze/process_social_media_to_bronze.py
        """
    )
    ...
    transform_to_silver = BashOperator(
        ...
        --conf spark.hadoop.fs.gs.impl=com.google.cloud.hadoop.fs.gcs.GoogleHadoopFileSystem \
        --jars /opt/spark/jars/delta-spark_2.13-3.3.0.jar,/opt/spark/jars/delta-storage-3.3.0.jar,/opt/spark/jars/gcs-connector-hadoop3-latest.jar \
        /opt/airflow/jobs/social_media/silver/transform_bronze_social_to_silver.py
        """
    )

    # 3️⃣ Transform từ Silver sang Gold
    transform_to_gold = BashOperator(
        ...
        --conf spark.hadoop.fs.gs.impl=com.google.cloud.hadoop.fs.gcs.GoogleHadoopFileSystem \
        --jars /opt/spark/jars/delta-spark_2.13-3.3.0.jar,/opt/spark/jars/delta-storage-3.3.0.jar,/opt/spark/jars/gcs-connector-hadoop3-latest.jar \
        /opt/airflow/jobs/social_media/gold/transform_silver_social_to_gold.py
        """
    )
